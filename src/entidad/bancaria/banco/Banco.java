...
public class Banco {
	...
	private static HashSet<CuentaDeCliente> cuentas;
	...
	public static void transferencia(int cbuDelOrigen, int cbuDelDestino, Double monto) 
		...
		if (cuentaDestino.getTipoDeMoneda() == cuentaOrigen.getTipoDeMoneda()) {
			...
			cuentaDestino.acreditar(monto, MotivoDeTransaccion.TRANSFERENCIA, "Conversión de Peso Argentino(ARS) a Dolar(USD): " + 
					monto + " Cotización: 1 ARS - " + Banco.getCotizacion()+ " USD");
		} else {
			monto = monto * Banco.getCotizacion();
			cuentaDestino.acreditar(monto, MotivoDeTransaccion.TRANSFERENCIA, "Conversión de Dolar(USD) a Peso Argentino(ARS): " +
					monto + " Cotización: 1 ARS - " + Banco.getCotizacion() + " USD");
		}
	}
	...
	public static Transaccion[] listarLosUltimosMovimientosDeCuenta(int cbu, int cantidadDeMovimientos)
		...
		for (int i = (cuenta.getTransacciones().size()) - cantidadDeMovimientos; i < cuenta.getTransacciones().size(); i++) {
	 ...
	 * @param cbu : CBU de la cuenta que se desea buscar.
	...
	public static void agregarPersonaFisica(String CUIT, String nombre,
		...
		Banco.clientes.add(new PersonaFisica(CUIT, nombre,
	...
	public static void agregarPersonaJuridica(String CUIT, String razonSocial,
		...
		Banco.clientes.add(new PersonaJuridica(CUIT, razonSocial,
